...
(Hey if you change to 'code' mode here, you will find a prolog playground!
that is under constructions now)
...
lie(bob) :- \+ lie(alice).
\break_window(cesi) :- \+ lie(cesi).

exactly_one_statement_holds :- 
  lie(A),
  lie(B),
  lie(C),
  A \= B,
...
something provable, as concise as possible. (Bonus: it is a key difference
